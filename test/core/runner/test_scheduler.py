...
# Sample Rules
rule1 = Rule(id="R1", description="", check="$ cmd1\n$ cmd2", fix="", severity="high")
rule2 = Rule(id="R2", description="Desc R2", check="$ cmd3", fix="Fix R2", severity="medium")
rule_short_many = Rule(id="R3", description="", check="\n".join([f"$ sleep 0.0{i}" for i in range(15)]), fix="", severity="low")
rule_mixed = Rule(id="R4", description="", check="$ long_cmd\n$ short1\n$ short2", fix="", severity="high")
rule_denied = Rule(id="R5", description="", check="$ rm -rf /\n$ ls", fix="", severity="critical")
...
@pytest.mark.parametrize("cmds, ests, expected_chunks", [
    ...
    # --- FIX THE EXPECTATION FOR THIS CASE ---
    (
        [f"c{i}" for i in range(15)], # Input cmds
        [0.1] * 15,                   # Input ests
        # Expected chunks (size 4 because max(ests) is NOT < 0.10)
        [['c0','c1','c2','c3'], ['c4','c5','c6','c7'], ['c8','c9','c10','c11'], ['c12','c13','c14']]
    ),
    # ----------------------------------------
    ([f"c{i}" for i in range(12)], [0.1] * 10 + [0.2, 0.2], [['c0','c1','c2','c3'], ['c4','c5','c6','c7'], ['c8','c9','c10','c11']]), # >=10, >=60% short, max_est > 0.1 -> chunk 4 (This case was already correct)
